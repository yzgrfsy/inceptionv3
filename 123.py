...
def clone_y_generator1(generator):
    while True:
        data = next(generator)
        x = [data[0]]
        y = [data[1], data[1]]
...
def clone_y_generator_(array):
    while True:
        ...
        labels = []
        for i in data[1]:
            labels.append(j for j in range(len(i)) if i[j]==1)
        '''
        x = [data[0], data[1]]
        # print(data[0])
        # print(data[1])
...
def override_keras_directory_iterator_next():
    ...
    def custom_next(self):
        ...
        #batch_x = batch_x[:, ::-1, :, :]
        batch_x = batch_x[:, :, :, ::-1]  #channel last
...
def get_classes_in_keras_format():
    ...
    #print("get_classes_in_keras_format:", dict(zip(config.classes, range(len(config.classes)))))
    return None


def get_model_class_instance(*args, **kwargs):
    module = importlib.import_module("models.{}".format(config.model))
    #print("model:", module.inst_class(*args, **kwargs))
