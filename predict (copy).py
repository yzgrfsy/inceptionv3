...
def predict(path):
    ...
    for n in range(0, nb_batch):
        ...
        if not args.store_activations:
            ...
            # intermediate_output = intermediate_layer_model.predict(np.array(inputs))
            # print("-------------------------------------------")
            # print(intermediate_output.shape)
            # fc1_out = np.squeeze(intermediate_output)
            # # fc1_out = np.squeeze(intermediate_output, axis=0)
            # print(fc1_out.shape)
            # # plt.imshow(fc1_out)
            # end = time.clock()
    ...
    if not args.store_activations:
        for i, p in enumerate(predictions):
            ...
            # print('files:', files[i])
            # os.sep means '/'
            print('| should be {} ({}) -> predicted as {} ({}) -> score : {}'.format(y_trues[i], files[i].split(os.sep)[-2], p,
