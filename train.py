...
def parse_args():
    ...
    parser.add_argument('--model', type=str, required=True, help='Base model architecture', choices=[
        ...
        config.MODEL_VGG16])
    parser.add_argument('--is_center_loss', help="use center loss", default=False)
    parser.add_argument("--is_triplet_loss", help="use triplet loss", default=False)
...
if __name__ == '__main__':
    try:
        ...
        if args.is_center_loss:
            config.isCenterLoss = args.is_center_loss
            print(config.isCenterLoss)
        if args.is_triplet_loss:
            config.isTripletLoss = args.is_triplet_loss
        init()
        train()
